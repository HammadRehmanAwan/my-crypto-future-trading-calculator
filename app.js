...
// Fetches intraday or hourly data for the forecast model.
// CoinGecko auto-selects granularity: days=1 → 5-min, days≤7 → hourly.
async function fetchForecastData(coinId, cgDays) {
  const cacheKey = `fcst-${coinId}-${cgDays}`;
  const parseRaw = raw => ({
    dates:  raw.prices.map(p => new Date(p[0])),
    prices: raw.prices.map(p => p[1]),
  });
  if (BACKEND_URL) {
    try {
      const raw = await backendFetch(
        `${BACKEND_URL}/cg/history/${coinId}?days=${cgDays}`,
        cacheKey, 60_000, { retries: 1, timeoutMs: 15_000 }
      );
      return parseRaw(raw);
    } catch (_) { /* fall through */ }
  }
  const url = `${BASE}/coins/${coinId}/market_chart?vs_currency=usd&days=${cgDays}`;
  return parseRaw(await apiFetch(url, cacheKey, 60_000));
}
...
// Format a Date for chart labels depending on data granularity.
...
async function loadCoin(coinId, days) {
  ...
  try {
    ...
    // Apply selected forecast horizon (may fetch intraday data)
...
async function analyze() {
  ...
  const m       = calcFutures(entry, lev, size, dir, tp, sl, fundingRate, horizDays);
  ...
  state.lastTrade = { m, curr, dir, score, hzCfg, chgF, lev };
  renderTradeCoach();
...
function renderResults(m, curr, fcst, hzCfg, chgF) {
  ...
  const predDesc = hzCfg.warning
    ? `Daily model (sub-hour trade analysis uses 7-day horizon for signal quality)`
    : (chgF >= 0
  ...
  const fundingPeriods = Math.round(hzCfg.horizDays * 3 * 100) / 100;
  const frSub  = m.fundingCost !== 0
    ? `${(Math.abs(state.futures?.funding_rate ?? 0) * 100).toFixed(4)}%/8h × ${fundingPeriods} periods (${hzCfg.label} hold)`
    : null;
